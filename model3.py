...
class IAN(object):
    ...
    def build_model(self):
        ...
        with tf.name_scope('weights'):
            weights = {
                'laspect_score': tf.get_variable(
                    name='W_la',
                    shape=[self.n_hidden*2, self.n_hidden*2],
                ...
                'softmax': tf.get_variable(
                    name='W_l',
                    shape=[self.n_hidden * 8, self.n_class],
                    initializer=tf.random_uniform_initializer(-0.1, 0.1),
                    regularizer=tf.contrib.layers.l2_regularizer(self.l2_reg)
                ),
            }
        ...
        with tf.name_scope('dynamic_rnn'):
            aspect_outputs, aspect_state = tf.nn.bidirectional_dynamic_rnn(
            ...
            aspect_state=tf.concat(aspect_state,1)
            batch_size = tf.shape(aspect_outputs)[0]
            aspect_avg = tf.reduce_mean(aspect_outputs, 1)

            lcontext_outputs, lcontext_state = tf.nn.bidirectional_dynamic_rnn(
                tf.contrib.rnn.GRUCell(self.n_hidden),
                tf.contrib.rnn.GRUCell(self.n_hidden),
            ...
            lcontext_outputs=tf.concat(lcontext_outputs,2)
            lcontext_state=tf.concat(lcontext_state,1)
            lcontext_avg = tf.reduce_mean(lcontext_outputs, 1)

            rcontext_outputs, rcontext_state = tf.nn.bidirectional_dynamic_rnn(
                tf.contrib.rnn.GRUCell(self.n_hidden),
                tf.contrib.rnn.GRUCell(self.n_hidden),
            ...
            rcontext_outputs=tf.concat(rcontext_outputs,2)
            rcontext_state=tf.concat(rcontext_state,1)
            ...
            def body(i, laspect_rep, laspect_att, raspect_rep, raspect_att):
                ...
                laspect_score = tf.reshape(tf.nn.tanh(tf.matmul(tf.matmul(a, weights['laspect_score']), tf.reshape(b1, [-1, 1])) + biases['laspect_score']), [1, -1])
                laspect_att_temp = tf.concat([tf.nn.softmax(tf.slice(laspect_score, [0, 0], [1, l])), tf.zeros([1, self.max_aspect_len - l])], 1)
                laspect_att = laspect_att.write(i, laspect_att_temp)
                laspect_rep = laspect_rep.write(i, tf.matmul(laspect_att_temp, a))

                raspect_score = tf.reshape(tf.nn.tanh(tf.matmul(tf.matmul(a, weights['raspect_score']), tf.reshape(b2, [-1, 1])) + biases['raspect_score']), [1, -1])
                raspect_att_temp = tf.concat([tf.nn.softmax(tf.slice(raspect_score, [0, 0], [1, l])), tf.zeros([1, self.max_aspect_len - l])], 1)
            ...
            def condition(i, laspect_rep, laspect_att, raspect_rep, raspect_att):
                return i < batch_size
            _, laspect_rep_final, laspect_att_final, raspect_rep_final, raspect_att_final  = tf.while_loop(cond=condition, body=body, loop_vars=(0, laspect_rep, laspect_att, raspect_rep, raspect_att))
            self.laspect_atts = tf.reshape(laspect_att_final.stack(), [-1, self.max_aspect_len])
            self.laspect_reps = tf.reshape(laspect_rep_final.stack(), [-1, self.n_hidden*2])
            self.raspect_atts = tf.reshape(raspect_att_final.stack(), [-1, self.max_aspect_len])
            self.raspect_reps = tf.reshape(raspect_rep_final.stack(), [-1, self.n_hidden*2])
            ...
            def body(i, lcontext_rep, lcontext_att, rcontext_rep, rcontext_att):
                ...
                lcontext_score = tf.reshape(tf.nn.tanh(tf.matmul(tf.matmul(a1, weights['lcontext_score']), tf.reshape(b, [-1, 1])) + biases['lcontext_score']), [1, -1])
                lcontext_att_temp = tf.concat([tf.nn.softmax(tf.slice(lcontext_score, [0, 0], [1, l1])), tf.zeros([1, self.max_context_len - l1])], 1)
                lcontext_att = lcontext_att.write(i, lcontext_att_temp)
                lcontext_rep = lcontext_rep.write(i, tf.matmul(lcontext_att_temp, a1))

                rcontext_score = tf.reshape(tf.nn.tanh(tf.matmul(tf.matmul(a2, weights['rcontext_score']), tf.reshape(b, [-1, 1])) + biases['rcontext_score']), [1, -1])
                rcontext_att_temp = tf.concat([tf.nn.softmax(tf.slice(rcontext_score, [0, 0], [1, l2])), tf.zeros([1, self.max_context_len - l2])], 1)
            ...
            def condition(i, lcontext_rep, lcontext_att, rcontext_rep, rcontext_att):
                return i < batch_size
            _, lcontext_rep_final, lcontext_att_final, rcontext_rep_final, rcontext_att_final = tf.while_loop(cond=condition, body=body, loop_vars=(0, lcontext_rep, lcontext_att, rcontext_rep, rcontext_att))
            self.lcontext_atts = tf.reshape(lcontext_att_final.stack(), [-1, self.max_context_len])
            self.lcontext_reps = tf.reshape(lcontext_rep_final.stack(), [-1, self.n_hidden*2])
            self.rcontext_atts = tf.reshape(rcontext_att_final.stack(), [-1, self.max_context_len])
            self.rcontext_reps = tf.reshape(rcontext_rep_final.stack(), [-1, self.n_hidden*2])
        ...
        _dir = 'logs2/' + str(timestamp) + '_r' + str(self.learning_rate) + '_b' + str(self.batch_size) + '_l' + str(self.l2_reg)
    ...
    def train(self, data):
        ...
        for sample, num in self.get_batch_data(aspects, lcontexts, rcontexts, labels, aspect_lens, lcontext_lens, rcontext_lens, self.batch_size, True, self.dropout):
...
#    def predict_sentiment(self, test_data):
#        with tf.Session() as sess:
#            new_saver = tf.train.import_meta_graph('models2/model_iter-3.meta')
#            new_saver.restore(sess,'models2/model_iter-3.data-00000-of-00001' )	
#            graph = tf.get_default_graph()
#            self.aspects=graph.get_tensor_by_name('self.aspects:0')
#            self.lcontexts=graph.get_tensor_by_name('self.lcontexts:0')
#            self.rcontexts=graph.get_tensor_by_name('self.rcontexts:0')
#            self.labels=graph.get_tensor_by_name('self.labels:0')
#            self.aspect_lens=graph.get_tensor_by_name('self.aspect_lens:0')
#            self.lcontext_lens=graph.get_tensor_by_name('self.lcontext_lens:0')
#            self.rcontext_lens=graph.get_tensor_by_name('self.rcontext_lens:0')
#            self.dropout_keep_prob=graph.get_tensor_by_name('self.dropout_keep_prob:0')
#            self.laspect_atts=graph.get_tensor_by_name('self.laspect_atts:0')
#            self.raspect_atts=graph.get_tensor_by_name('self.raspect_atts:0')
#            self.lcontext_atts=graph.get_tensor_by_name('self.lcontext_atts:0')
#            self.rcontext_atts=graph.get_tensor_by_name('self.rcontext_atts:0')
#            self.correct_pred=graph.get_tensor_by_name('self.correct_pred:0')
#            self.cost=graph.get_tensor_by_name('self.cost:0')
#            self.accuracy=graph.get_tensor_by_name('self.accuracy:0')
#
#
#
#            timestamp = str(int(time.time()))
...
#            cost, acc, cnt = 0., 0, 0
#            with open('predict2/test_' + str(timestamp) + '.txt', 'w') as f:
#                for sample, num in self.get_batch_data(aspects, lcontexts, rcontexts, labels, aspect_lens, lcontext_lens, rcontext_lens, len(aspects), False, 1.0):
#                    laspect_atts, raspect_atts, lcontext_atts, rcontext_atts, correct_pred, cost, accuracy = sess.run([self.laspect_atts, self.raspect_att, self.lcontext_atts, self.rcontext_atts, self.correct_pred, self.cost, self.accuracy], feed_dict=sample)
#                    cost += loss * num
#                    acc += accuracy
#                    cnt += num
...
#                        f.write('%s\n%s\n%s\n%s\n%s\n' % (a, b, c, d, e))
#                f.write('%f\n%f\n' % (cost/cnt,acc/cnt))
#            print('Finishing predicting testing data')
    ...
    def get_batch_data(self, aspects, lcontexts, rcontexts, labels, aspect_lens, lcontext_lens, rcontext_lens, batch_size, is_shuffle, keep_prob):
