...
class IAN(object):
    ...
    def build_model(self):
        with tf.name_scope('inputs'):
            ...
            self.lcontexts = tf.placeholder(tf.int32, [None, self.max_context_len])
            self.rcontexts = tf.placeholder(tf.int32, [None, self.max_context_len])
            self.labels = tf.placeholder(tf.int32, [None, self.n_class])
            self.aspect_lens = tf.placeholder(tf.int32, None)
            self.lcontext_lens = tf.placeholder(tf.int32, None)
            self.rcontext_lens = tf.placeholder(tf.int32, None)
